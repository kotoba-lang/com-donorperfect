(ns donorperfect.main
  "Kotodama WASM entrypoint for the Donorperfect clean-room actor (L4) — Clojure port.
  ...
  facts (`donorperfect.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "donorperfect")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"donorperfect.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "donorperfect-compat" :tier tier :entities entities} 200])
...
(defn start! [] :donorperfect-compat/ready)
